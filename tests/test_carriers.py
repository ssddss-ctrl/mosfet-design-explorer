...
Numerical tolerances are calibrated to the textbook's ni = 1.5e10 cm^-3
...
class TestIntrinsicConcentration:
    ...
    def test_ni_300K_in_physical_range(self):
        """
        ni at 300 K should match the textbook value of 1.5e10 cm^-3
        (Streetman eq. 3-23, p.116) to within 20%.
        """
        result = ni(300.0)
        assert 1.2e10 < result < 1.8e10, (
            f"ni(300) = {result:.3e} cm^-3, expected ~1.5e10"
        )

    def test_ni_300K_exact(self):
        """
        ni(300) must equal ni_300 exactly, since the scaling formula
        is anchored there (T/300 = 1, Boltzmann exponent = 0).
        """
        result = ni(300.0)
        assert abs(result - C.ni_300) / C.ni_300 < 1e-9, (
            f"ni(300) = {result:.6e}, expected {C.ni_300:.6e}"
        )
    ...
    def test_ni_300K_mass_action_self_consistent(self):
        ...
        to within 1%, confirming the quadratic solver and mass-action
        law are consistent.
        """
...
class TestMassActionLaw:
    ...
    def test_mass_action(self, Na, Nd):
        """n0 * p0 must equal ni² to machine precision (Streetman eq. 3-24)."""
...
class TestExtrinsicApproximation:
    ...
    def test_p_type_extrinsic_1e17(self):
        """
        For Na = 1e17 >> ni = 1.5e10, p0 should be ≈ Na to within 0.1%.
        The exact quadratic solution includes a small ni²-dependent
        correction that causes p0 to exceed Na by ~0.09%.
        """
        Na = 1e17
        _, p0 = carrier_concentrations(Na=Na, Nd=0.0, T=300.0)
        assert abs(p0 / Na - 1.0) < 1e-3, (
            f"p0/Na = {p0/Na:.6f}, expected ≈ 1.0 ± 0.1%"
        )
...
class TestFermiLevel:
    ...
    def test_EF_model_consistent_value(self):
        """
        EF − Ei at Nd = 1e17 should match Streetman Example 3-5:
        kT*ln(1e17 / 1.5e10) = 0.0259 * ln(6.67e6) = 0.407 eV.
        """
        EF_Ei = fermi_level(Na=0.0, Nd=1e17)
        assert abs(EF_Ei - 0.407) < 0.002, (
            f"EF−Ei = {EF_Ei:.4f} eV, expected 0.407 eV (Streetman Ex. 3-5)"
        )
...
class TestMobility:
    ...
    def test_mobility_undoped_returns_lattice_limit(self):
        ...
        effectively the lattice-limited value (Streetman §3.4.3).
        """
